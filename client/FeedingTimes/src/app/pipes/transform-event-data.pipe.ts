...
export class TransformEventDataPipe implements PipeTransform {
  transform(value: UserEvent | Feeding, ...args: unknown[]): Activity {
    ...
    if ('quantity' in value) {
      output = {
        type: value.type ? value.type : 'not found',
        info: `quantity: ${value.quantity} ml`,
        time: value.time ? value.time : 'not found',
        iconPath: value.icon
          ? iconTransform.transform(value.icon)
          : 'not found',
      };
    } else if ('description' in value) {
      output = {
        type: value.type ? value.type : 'not found',
        info: `desciption: ${value.description}`,
        time: value.time ? value.time : 'not found',
        iconPath: value.icon
          ? iconTransform.transform(value.icon)
          : 'not found',
